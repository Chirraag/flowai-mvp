...
const BusinessNode: React.FC<NodeProps> = ({ data }) => {
  ...
  const safeNodeType = typeof nodeType === 'string' ? nodeType : 'Business Node';
  const safeName = typeof name === 'string' ? name : '';
  const safeDescription = typeof description === 'string' ? description : '';

  return (
    <div
      className="relative group bg-gradient-to-br from-purple-50 to-violet-50 text-gray-800 rounded-xl min-w-[180px] max-w-[260px] text-left border-2 border-purple-400 font-semibold shadow-lg p-4 hover:border-purple-500 hover:shadow-xl transition-all duration-200"
    ...
    >
      ...
      {/* Node Type Badge */}
      <div className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-semibold bg-purple-500 text-white border border-purple-600 mb-3 shadow-sm">
        <Cog className="w-3 h-3" />
        {safeNodeType}
      </div>
      
      {/* Node Name */}
      <div className="text-base font-bold mb-2 break-words leading-tight text-purple-800">
        {safeName}
      </div>
